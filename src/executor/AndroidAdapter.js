...
class AndroidAdapter extends AutoIQExecutor {
  constructor(options = {}) {
    super();
    this.options = {
  ...
  async initialize() {
    const capabilities = {
      ...
      "appium:appActivity": this.options.appActivity,
      "appium:noReset": true,
  ...
  async apiRequest() {
    throw new Error("apiRequest is not supported in AndroidAdapter. Use APIAdapter instead.");
  }
  ...
  ensureInitialized() {
    if (!this.driver) throw new Error("AndroidAdapter is not initialized. Call initialize() first.");
  }
  ...
  toBy(locator) {
    ...
    if (normalized.startsWith("text=")) {
      ...
      // Support Android specific attributes: text, content-desc and standard web text
      return By.xpath(`//*[@text=${literal} or @content-desc=${literal} or contains(text(), ${literal})]`);
    }

    if (normalized.startsWith("xpath=")) return By.xpath(trimmed.slice(6).trim());

    if (normalized.startsWith("name=")) {
      return By.name(trimmed.slice(5).trim());
    }

    if (normalized.startsWith("id=")) return By.id(trimmed.slice(3).trim());
    if (trimmed.startsWith("//")) return By.xpath(trimmed);
